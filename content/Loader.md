...
| ------------------------------------ | -------------------------------------------------------------------------------------------- |
| **Compile and Go Loader**            | The assembler loads the machine code directly into memory; no object file is created.        |
| **[[Absolute Loader]]**              | Loads programs at specified memory addresses. Allocation and linking are manual.             |
| **Relocating Loader**                | Supports relocation, enabling flexible memory assignment.                                    |
| ** [[Direct Linking Loader]] (DLL)** | Uses two passes to resolve external references and relocation. Builds a load module.         |
| **Dynamic Loading Loader**           | Loads only required segments at runtime, using an overlay structure for memory optimization. |
| **[[Dynamic Linking Loader]]**       | Loads and links modules during execution time (only when referenced).                        |
